-- student 학생 number(3), varchar2(20) : 문자열
...
math number(3),
total number(3),
avg number(6,2), -- 소숫점 2째자리 까지 (전체자리, 소수점 n째자리)
rank number(4) -- 4자리 (0000~9999)
);
...
insert into student(no, name, kor, eng, math, total, avg, rank)
values(1, '홍길동', 100, 100, 99, 299, (299/3), 1);

insert into student(no, name, kor, eng, math, total, avg, rank)
values(2, '유관순', 80, 80, 85, 245, (245/3), 0);

insert into student(no, name, kor, eng, math, total, avg, rank)
values(3, '이순신', 90, 90, 91, 271, (271/3), 0);

insert into student(no, name, kor, eng, math, total, avg, rank)
values(4, '강감찬', 60, 65, 67, 192, (192/3), 0);

insert into student(no, name, kor, eng, math, total, avg, rank)
values(5, '김구', 100, 100, 84, 284, (284/3), 0);

insert into student values(
6,'홍길자', 100, 100, 90, (100+100+90), ((100+100+90)/3),0);

select * from tab; -- 현재 생성된 테이블보기

desc student; -- 테이블 타입 확인

create table no_tab (
no number,
name varchar2(20),
avg1 number,
avg2 number(5,2) -- 소숫점 둘째자리에서 반올림
);
...
1, '2024-09-28',sysdate -- sysdate : 현재시각(시분초까지 가능)
);

select * from date_tab;
select to_char(s_date, 'YYYY-MM-DD'),to_char(s_date2, 'yyyy-mm-dd hh24:mi:ss') from date_tab;

select to_char(s_date, 'yyyy-mm-dd hh24:mi:ss') from date_tab;

-- varchar2(n) : 가변길이(n) - 최대 n까지 저장할 수 있음
-- char : 길이가 정해져있음

-- 검색 : select 컬럼명 from 테이블명 (컬럼명 * : 모든컬럼)
select * from employees;
select employee_id,emp_name,salary from employees; -- 아이디 이름 급여 까지
select EMP_NAME from employees;

SELECT EMP_NAME FROM EMPLOYEES;
select emp_name from employees; -- 대소문자 구별 x

select emp_name from employees where emp_name = 'Pat Fay'; -- 밸류는 대소문자 구분
...
-- 데이터 삭제 : delete
delete member where id='Trineman'; -- 조건절 (where) : 컬럼에 해당되는 값
...
-- male 이면서 golf가 취미인사람 삭제

delete member where gender = 'Male' or hobby= 'golf';
-- male 이거나 golf가 취미인사람 삭제
